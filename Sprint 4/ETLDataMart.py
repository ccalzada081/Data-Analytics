...
SQL_USERNAME = "sa"
SQL_PASSWORD = "Academic2025U04!"   # <-- keep this private in real life

# We will connect to WarehouseDB, but all queries are fully qualified
...
# -------------------------------------------------------------------
# 3. ETL logic for DataMart (full refresh)
# -------------------------------------------------------------------
def load_datamart(cursor):
    ...
    print("\n[STEP] Rebuilding DataMart (full refresh)...")
    ...
    print(f"[INFO] DataMart loaded successfully. Total rows: {total_rows}")
    return total_rows
...
def start_etl_run(cursor, job_name):
    ...
    print(f"[DG] Started ETL run. Run_ID = {run_id}")
    return run_id
...
def finish_etl_run(cursor, run_id, status, duration_seconds,
    ...
    print(f"[DG] ETL run {run_id} finished with status = {status}")
...
def run_validations(cursor, run_id):
    ...
    """
    print("\n[STEP] Running data quality validations...")

    # If Is_Active does not exist or is NULL, we still run the rule
    sql_rules = """
    ...
    for rule_id, rule_name, severity, rule_logic in rules:
        ...
        try:
            ...
            # If the rule SQL itself errors, we log as failed with special note
            print(f"    ! Error executing rule {rule_name}: {e}")
            failed_count = -1  # -1 = execution error
    ...
    print(f"[DG] Validation summary: {total_failed_all} failing rows in total.")
    print(f"[DG] Critical failures: {total_failed_critical}")
...
def main():
    ...
    try:
        ...
        # If there are critical failures, mark status as Failed (or Partial)
        ...
        # If something goes wrong before finishing, we try to log it
        duration_seconds = int(time.time() - start_time)
        ...
        try:
            # We open a new connection to log the failure, if possible
